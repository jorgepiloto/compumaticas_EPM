# Titulo: Noche estrellada con Python turtle Autor: Jorge Martínez Garrido

# Una de las cosas que más me gustan es ver las estrellas por la noche. Un cielo
# de color negro lleno de pequeños puntos blancos me resulta algo precioso. ¿Y
# si dibujamos una noche estrellada con Python y turtle?
#
# Como siempre, el código fuente[^1] puedes encontrarlo al final del post.

# ## Atacando el problema
#
# Para visualizar mejor el problema, imaginemos un grupo de estrellas dispuestas
# en el cielo nocturno:
#
# ![Esquema noche estrellada](img/stars_background_auxiliary.png)
#
# A lo largo del tiempo, diversas culturas han agrupado las estrellas en formas
# conocidas bajo el nombre de constelaciones. En la imagen anterior se muestra
# la popular Osa Mayor. Nuestro objetivo es dibujar solamente las estrellas, no
# las líneas o las constelaciones.
#
# Para ello, lo que podemos hacer es:
#
# 1. Dibujar un fondo negro que simule el espacio sideral y la noche.  2.
# Generar puntos aleatorios con coordenadas x e y donde situar cada una de las
# estrellas.  3. Generar un radio aleatorio para cada estrella, lo que dará
# mayor realismo a la imagen.
#
# En este tutorial asumiremos que todas las estrellas son de color blanco, pero
# puedes animarte a darles otros colores como por ejemplo amarillo o azul.
...
def dibujar_cielo_nocturno(base, altura, color=(0, 0, 0)):
    ...
    lista_puntos = [(b_2, a_2), (-b_2, a_2), (-b_2, -a_2), (b_2, -a_2), (b_2, 0)]
...
#
# Una vez tenemos definida la rutina para crear el cielo nocturno es hora de
# crear una nueva que dibuje cierto número de estrellas sobre dicho fondo. Las
# estrellas deben de ser círculos blancos y el valor de sus radios oscila entre
# ciertos límites. Además, la posición de una estrella debe de ser aleatoria
# pero quedar enmarcada dentro del fondo creado anteriormente.

# Vamos a reutilizar la función `dibujar_anillo` del post [Dibujando el logo de
# los Juegos Olímpicos con Python
# turtle](https://jorgemartinez.space/posts/compumaticas/dibujando-el-logo-de-los-juegos-ol%C3%ADmpicos-con-python-turtle/).
# La llamaremos `dibujar_estrella` y la modificaremos para que en lugar de
# dibujar un anillo, dibuje un círculo relleno de color.
...
# Sabemos que podemos generar números aleatorios comprendidos entre $0$ y $1$.
# Podemos entender un valor en el rango $(0, 1)$ como un coeficiente $\eta$ que
# escala un determinado valor. Si $\eta=0$, entonces $\eta \cdot L = 0$. Por
# otro lado, si $\eta = 1$, entonces $\eta \cdot L = L$, lo que implica que para
# cualquier valor de $\eta$ entre $0$ y $1$ implica que el producto $\eta \cdot
# L$ tendrá un valor comprendido entre $0$ y $L$.
#
# Es importante darse cuenta de que el origen de coordenadas de turtle es el
# centro de la ventana y no la esquina de inferior izquierda de la misma. Por
# ello, debemos restar la mitad de la base y altura del rectángula a la hora de
# generar las coordenadas aleatorias de los puntos.
...
#
# Una vez redactadas todas las funciones anteriores, es posible generar el
# siguiente código para conseguir dibujar nuestro fondo estrellado. Para lograr
# un mejor efector, vamos a esconder la "tortuga" utilizando
# `turtle.hideturtle()` y a darle la máxima velocidad posible aplicando
# `turtle.speed(0)`.
...
# ## Resultado
#
# Dado que estamos trabajando con números aleatorios, cada resultado será
# diferente salvo que fijemos la conocida como "semilla" o `seed`. De cualquier
# modo, ejecutando el código anterior deberías de obtener algo similar a la
# siguiente animación:
...
# fuente](https://github.com/jorgepiloto/compumaticas_EPM/blob/main/compumaticas_0/stars_background/main.py)
